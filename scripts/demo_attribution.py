...
DEMO_MEASUREMENT_LAYER = (
    None  # None = last layer (unembed); set to int for intermediate
)
DEMO_TOP_K = 20
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--model", type=str, default=DEMO_MODEL_NAME, help="HuggingFace model name"
    )
    p.add_argument(
        "--transcoder", type=str, default=DEMO_TRANSCODER, help="Transcoder repo/preset"
    )
    p.add_argument(
        "--backend",
        type=str,
        default=DEMO_BACKEND,
...
def load_contrastive_prompts(n_prompts: int) -> tuple[list[str], list[str]]:
    ...
    log.info(
        "  Loaded %d harmful, %d harmless prompts",
        len(harmful_texts),
        len(harmless_texts),
    )
...
def run_attribution(
    ...
    *,
    ...
    graph = attribute_to_direction(
        ...
        direction=r_hat.to(
            replacement_model.device if hasattr(replacement_model, "device") else "cuda"
        ),
...
def compare_features(
    ...
    suppression_candidates = [f for f in jailbreak_features if f["attribution"] < -0.01]
...
def print_feature_table(features: list[dict], title: str) -> None:
    ...
    print(f"{'=' * 78}")
    print(
        f"  {'Rank':>4}  {'Layer':>5}  {'Pos':>4}  {'Feature':>8}  {'Attribution':>12}  {'Activation':>10}"
    )
...
def make_plots(
    ...
    if len(shared_keys) >= 3:
        ...
        ax.set_title(
            f"Shared Features: Harmful vs Jailbreak Attribution (n={len(shared_keys)})"
        )
        ax.legend()
        plt.tight_layout()
...
def main() -> None:
    ...
    # Step 5: Output
    print_feature_table(
        agg_harmful, f"Aggregated Top Features: Bare Harmful ({n_pairs} prompts)"
    )
    print_feature_table(
        agg_jailbreak, f"Aggregated Top Features: RP Jailbreak ({n_pairs} prompts)"
    )
    ...
    print(f"{'=' * 78}")
    print(
        f"  {'Pair':>4}  {'Harmful Sum':>12}  {'Jailbreak Sum':>14}  {'Reduction':>10}  {'Neg(H)':>6}  {'Neg(J)':>6}"
    )
    ...
    print(
        f"  Suppression candidates (avg A < 0 on jailbreak): {len(comparison['suppression_candidates'])}"
    )
