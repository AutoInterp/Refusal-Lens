...
#   export RUN_DIR=data/results/pipeline_runs/run_20260422_015552
#   export BRANCH=l15-refactor              # branch to commit/push to
#   bash scripts/pipeline/run_stage08_overnight.sh
#
...
: "${RUN_DIR:?must set RUN_DIR=data/results/pipeline_runs/run_<ts>}"
...
tmux new-session -d -s "$SESSION" -n run \
  "cd $REPO && source $VENV_ACTIVATE && \
   PYTHONPATH=src python3 -u scripts/pipeline/08_ablate_subcircuits.py \
     --run-dir $RUN_DIR \
     --positions both \
     --max-new-tokens 80 \
     --skip-baseline \
