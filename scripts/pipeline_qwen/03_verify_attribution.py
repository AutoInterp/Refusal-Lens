"""
Stage 03: Verify Attribution Gap (Qwen3-4B)
============================================
...
where circuit-tracer measures. For Qwen3 with feature_input_hook="mlp.hook_in"
this is the OUTPUT of `post_attention_layernorm` of the measurement layer
(post-RMSNorm, pre-MLP — Qwen3's standard pre-LN block has the second LN
sitting between attention and MLP). The Gemma analogue was
`pre_feedforward_layernorm` (Gemma-3's quad-LN block). Using
`out.hidden_states[L+1]` (the post-block residual) instead would give a
nonsense ratio — that is the failure mode of MENTEE_NOTE Bug 1.

Computes:
1. Direct dot <r_hat, post_attention_layernorm.output[L][pos]> for each prompt
...
def main():
    ...
    # ----------------------------------------------------------
    # Resolve the measurement-point hook path (Qwen3 standard pre-LN block).
    # circuit-tracer's transcoder is configured with feature_input_hook=
    # "mlp.hook_in", which on Qwen3 resolves to the OUTPUT of
    # `post_attention_layernorm` at the measurement layer (the LN sitting
    # between attention and MLP). We hook the same module here so direct
    # dot and attribution sum are comparable at the SAME residual point.
    # ----------------------------------------------------------
    def _get_layer_module(m, layer_idx):
        # Qwen3ForCausalLM: model.model.layers[L]
        # (handle Gemma fallbacks too in case someone repoints config.MODEL_NAME)
    ...
    layer_module = _get_layer_module(model, best_layer)
    if hasattr(layer_module, "post_attention_layernorm"):
        measurement_module = layer_module.post_attention_layernorm
        hook_name = "post_attention_layernorm"
    elif hasattr(layer_module, "pre_feedforward_layernorm"):
        # Gemma-3 quad-LN block — supported as fallback for cross-model use.
        measurement_module = layer_module.pre_feedforward_layernorm
        hook_name = "pre_feedforward_layernorm"
    else:
        raise RuntimeError(
            "Layer module exposes neither post_attention_layernorm nor "
            "pre_feedforward_layernorm; cannot resolve measurement point."
        )
    print(f"  Measurement hook: {hook_name}.output of layer {best_layer}")
    ...
    for i, entry in enumerate(prompts_and_nets):
        ...
        # Capture residual at the measurement-point LN output via forward hook
    ...
    print(f"  measurement point: {hook_name}.output[L={best_layer}][pos={best_pos}]")
    print(f"{'='*60}")
