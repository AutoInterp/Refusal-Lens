#!/usr/bin/env bash
# scripts/pipeline_qwen/run_p7.sh
#
...
#   bash scripts/pipeline_qwen/run_p7.sh          # default: smoke+full+push, in tmux
#
# Common variants:
#   bash scripts/pipeline_qwen/run_p7.sh --mode smoke           # smoke only (~1.5 h)
#   bash scripts/pipeline_qwen/run_p7.sh --mode full            # skip smoke (~22-26 h)
#   bash scripts/pipeline_qwen/run_p7.sh --no-tmux              # current shell
#   bash scripts/pipeline_qwen/run_p7.sh --git-push-results     # commit small results
#   bash scripts/pipeline_qwen/run_p7.sh --no-push-raw          # skip raw .pt push
#
# Logs / markers:
...
trap '
    ...
    if [[ $rc -ne 0 ]]; then
        ...
        echo "  2. Re-run from scratch:      bash scripts/pipeline_qwen/run_p7.sh --mode <smoke|full|both>"
...
if [[ "$MODE" == "smoke" || "$MODE" == "both" ]]; then
    ...
    run_stage "01_smoke" python3 scripts/pipeline_qwen/01_compute_direction.py \
        --run-dir "$SMOKE"

    run_stage "02_smoke" python3 scripts/pipeline_qwen/02_run_attribution.py \
        --run-dir "$SMOKE" --n-prompts $SMOKE_PROMPTS --batch-size $BATCH_SIZE \
        --save-graphs

    run_stage "03_smoke_multi" python3 scripts/pipeline_qwen/03_verify_attribution.py \
        --run-dir "$SMOKE" --graph-mode multi
    run_stage "03_smoke_single" python3 scripts/pipeline_qwen/03_verify_attribution.py \
        --run-dir "$SMOKE" --graph-mode single

    run_stage "02b_smoke" python3 scripts/pipeline_qwen/02b_statistical_analysis.py \
        --run-dir "$SMOKE"

    run_stage "04_smoke" python3 scripts/pipeline_qwen/04_label_features.py \
        --run-dir "$SMOKE"

    run_stage "02c_smoke" python3 scripts/pipeline_qwen/02c_pack_graphs.py \
        --run-dir "$SMOKE"

    run_stage "07_smoke" python3 scripts/pipeline_qwen/07_identify_subcircuits.py \
        --run-dir "$SMOKE"

    run_stage "06_smoke" python3 scripts/pipeline_qwen/06_causal_intervention.py \
        --run-dir "$SMOKE" --max-prompts $SMOKE_PROMPTS

    run_stage "08_smoke" python3 scripts/pipeline_qwen/08_ablate_subcircuits.py \
...
if [[ "$MODE" == "smoke" ]]; then
    ...
    echo "  bash scripts/pipeline_qwen/run_p7.sh --mode full"
    echo "============================================================"
    exit 0
fi
...
run_stage "01" python3 scripts/pipeline_qwen/01_compute_direction.py --run-dir "$RUN"

run_stage "02" python3 scripts/pipeline_qwen/02_run_attribution.py \
    --run-dir "$RUN" --n-prompts $FULL_PROMPTS --batch-size $BATCH_SIZE \
    --save-graphs

run_stage "02b" python3 scripts/pipeline_qwen/02b_statistical_analysis.py --run-dir "$RUN"
run_stage "03_multi"  python3 scripts/pipeline_qwen/03_verify_attribution.py --run-dir "$RUN" --graph-mode multi
run_stage "03_single" python3 scripts/pipeline_qwen/03_verify_attribution.py --run-dir "$RUN" --graph-mode single
run_stage "04"  python3 scripts/pipeline_qwen/04_label_features.py --run-dir "$RUN"
run_stage "02c" python3 scripts/pipeline_qwen/02c_pack_graphs.py    --run-dir "$RUN"
run_stage "07"  python3 scripts/pipeline_qwen/07_identify_subcircuits.py --run-dir "$RUN"

run_stage "06"  python3 scripts/pipeline_qwen/06_causal_intervention.py \
    --run-dir "$RUN" --max-prompts $FULL_PROMPTS

run_stage "08" python3 scripts/pipeline_qwen/08_ablate_subcircuits.py \
...
if [[ $PUSH_RAW -eq 1 ]]; then
    run_stage "push_raw" python3 scripts/pipeline_qwen/push_raw_graphs.py \
        --run-dir "$RUN" --dataset-repo "$DATASET_REPO"
fi

if [[ $PUSH_PACKED -eq 1 ]]; then
    run_stage "push_packed" python3 scripts/pipeline_qwen/push_graph_data.py \
        --run-dir "$RUN" --source 02c --dataset-repo "$DATASET_REPO"
fi

if [[ $PUSH_RUN_META -eq 1 ]]; then
    run_stage "push_run_meta" python3 scripts/pipeline_qwen/push_run.py \
