...
Two position modes:
  ...
  anchors — intervention applied only at template-anchor positions
...
def _extract_response(full_decoded: str, formatted: str) -> str:
    ...
    Fallback markers are tried in order: Qwen3 then Gemma-3, since the same
    Stage 08 utilities can be invoked from either pipeline. The marker is
    only used when the prefix-strip fails (e.g. extra whitespace from
