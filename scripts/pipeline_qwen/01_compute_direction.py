"""
Stage 01: Compute Per-Layer Refusal Directions (Qwen3-4B)
==========================================================
Sibling of `scripts/pipeline/01_compute_direction.py` (Gemma-3-4b-it). Same
methodology — diff-in-means at every decoder layer + per-position sweep at
the causal layer — Qwen-specific divergences:
  * 36 layers instead of 34.
  * Flat HuggingFace config (`model.config.hidden_size`) instead of Gemma's
    nested `model.config.text_config.hidden_size`. Both paths are handled
    by `utils.get_hidden_size`.
  * Chat template: `<|im_start|>assistant\\n` with enable_thinking=False
    (handled by `utils.format_prompt`).

Per-layer directions are critical: the direction rotates across layers, and
...
Outputs: 01_direction/directions/layer_XX.pt    (per-layer normalized r_hat)
         01_direction/unnormalized_r.pt         (per-layer unnormalized r — for Stage 06)
         01_direction/positions_L{XX}/pos_*.pt  (per-position at causal layer — for Stage 02 multi)
         01_direction/refusal_direction.pt      (legacy single-file format)
         01_direction/direction_metadata.json   (separations, cosines, position curve)
"""
...
import config
from utils import format_prompt, save_json, get_stage_dir, get_hidden_size
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Specific layers to compute (default: all 36 for Qwen3-4B)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"Layer at which to compute per-position refusal directions "
             f"(Qwen default: {config.PER_POSITION_LAYER}, = BEST_CAUSAL_LAYER).",
    )
    parser.add_argument(
...
def compute_mean_activations(model, tokenizer, prompts, layers, batch_size=4):
    """
    Compute mean activation at specified layers, position config.DIRECTION_POSITION,
    float64 accumulation.

    Diff-in-means methodology: fixed trailing position (negative indexing),
    batched, float64 for precision. The Qwen default position is -1 (last
    token after `<|im_start|>assistant\\n` with thinking disabled); Gemma's
    analog was -2 ("model" token). Override via config.DIRECTION_POSITION.
    """
    d_model = get_hidden_size(model)
    means = {layer: torch.zeros(d_model, dtype=torch.float64) for layer in layers}
    n = len(prompts)
    pos_offset = config.DIRECTION_POSITION  # negative, e.g. -1
    ...
    for i in range(0, n, batch_size):
        ...
        for j in range(len(batch)):
            for layer in layers:
                # Trailing position from chat template (Qwen default: -1).
                pos = input_ids.shape[1] + pos_offset
...
def compute_per_position_means(
    ...
    `positions` is a list of negative ints (from the end of the real prompt;
    on Qwen3 with enable_thinking=False, -1 is the trailing `\\n`, -2 is
    `assistant`, -4 is `<|im_start|>`, etc.). For each prompt, positions
    outside the prompt's real token span are skipped — the means are
