...
at ALL 36 Qwen3 layers using difference-in-means (Arditi et al., 2024).
...
- Attribution uses BEST_SEPARATION_LAYER, intervention uses BEST_CAUSAL_LAYER.
- Both are placeholders in config.py until this stage validates them; pick
  the position via the qwen_experiments position sweep first, then set
  config.DIRECTION_POSITION before running this stage.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Specific layers to compute (default: all 36)",
    )
    parser.add_argument(
...
def get_hidden_size(model) -> int:
    """Read d_model — Qwen3 has flat config, Gemma3 nests in text_config."""
...
def compute_mean_activations(model, tokenizer, prompts, layers, batch_size=4):
    ...
    Methodology: fixed trailing position (negative indexing), batched, float64
    for precision. Position default is -1 for Qwen3 (last token after
    `<|im_start|>assistant\\n` with thinking disabled); change via config.
    """
    d_model = get_hidden_size(model)
    means = {layer: torch.zeros(d_model, dtype=torch.float64) for layer in layers}
    n = len(prompts)
    pos = config.DIRECTION_POSITION  # negative index, e.g. -1
...
def main():
    ...
    # Indices span low / mid / late / final regimes of Qwen3-4B's 36-layer
    # stack — adjust once the separation curve is known.
    key_layers = [l for l in [15, 18, 27, 34] if l in layers]
    cosine_sims = {}
