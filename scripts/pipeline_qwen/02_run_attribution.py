"""
Stage 02: Run CLT Attribution Graphs (Qwen3-4B)
================================================
Sibling of `scripts/pipeline/02_run_attribution.py` (Gemma-3-4b-it). Same
methodology, same circuit-tracer call, same output schema — Qwen-specific
divergences flagged below. Re-read those before tweaking constants.

Differences from the Gemma version:
  * MEASUREMENT_LAYER default is 34 (Qwen has 36 layers; placeholder until
    01b layer-sweep + position sweep complete).
  * MEASUREMENT_POSITION default is -1 (Qwen template ends in `\\n` after
    `<|im_start|>assistant`; Gemma's pos=-2 ("model" token) ≈ Qwen's pos=-2
    (`assistant` token). Verify via the Stage 01 position sweep.)
  * TARGET_POSITIONS_MULTI defaults to [-5, -3, -1] (Qwen template anchors;
    Gemma uses [-5, -3, -2]).
  * format_prompt uses Qwen's chat template with enable_thinking=False.
  * Model+transcoder loaded by circuit-tracer via the vendored Qwen patch
    (refusal-lens-residual-stream-hook branch).

Dataset: shared 50 prompts × 11 conditions controlled dataset
(refusal_lens_controlled_dataset.json). Re-validate refusal/comply rates
on Qwen via verify_dataset_qwen.py before relying on the JSON's metadata.

Extracts ALL active features per graph (no top-k filtering) and performs
3-way feature comparison: bare↔JB, bare↔ctrl, ctrl↔JB.

Inputs:  01_direction/directions/layer_{TARGET}.pt
         01_direction/positions_L{TARGET}/pos_*.pt   (multi-target mode)
...
Outputs: 02_attribution/attribution_results.json,
         ...
         02_attribution/graphs/*.pt                  (when --save-graphs)
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Positions for the multi-target attribution graph (default: "
             "the Qwen3 template anchors [-5, -3, -1] — re-tune after "
             "the Qwen position sweep). Requires Stage 01 run with "
             "per-position directions at target_layer.",
    )
    parser.add_argument(
        "--single-position-target", type=int,
        default=config.TARGET_POSITIONS_SINGLE[0],
        help="Position for the single-target baseline graph (Qwen default: "
             "-1, matches Stage 01 DIRECTION_POSITION). Re-tune after the "
             "position sweep verifies which trailing token carries the "
             "refusal signal on Qwen.",
    )
    parser.add_argument(
...
def main():
    ...
    if produce_single:
        ...
        print(
            ...
            f"{modes['single']['positions']} (Qwen pos=-1 trailing newline)"
        )
