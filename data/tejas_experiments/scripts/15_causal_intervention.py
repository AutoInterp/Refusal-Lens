...
d = torch.load("/workspace/Refusal-Lens/data/tejas_experiments/results_v2/refusal_direction_v2.pt", map_location="cpu")
...
for cls_name in JB_CLASSES:
    for layer in INTERVENTION_LAYERS:
        ...
        total = len(cls_layer)
        print(f"  {cls_name:<18} L{layer:<6} {baseline_comply}/{total:<16} {flipped}/{baseline_comply if baseline_comply else total:<8} {coherent_flip}")
