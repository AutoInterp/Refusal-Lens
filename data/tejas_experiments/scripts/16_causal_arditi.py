...
for prompt in BENIGN_PROMPTS:
    ...
    for layer in TEST_LAYERS:
        try:
            ...
            print(
                f"    L{layer:>2} +r(all pos): {cls_int} coherent={coherent} | {resp_int[:60]}..."
            )
    ...
    for i, prompt in enumerate(test_prompts):
        ...
        for layer in TEST_LAYERS:
            try:
                ...
                print(
                    f"    L{layer:>2}: {cls_jb}->{cls_int} coherent={coherent}{marker}"
                )
...
print(
    f"|r| magnitudes: {', '.join(f'L{l}={unnormalized_r[l].norm().item():.1f}' for l in TEST_LAYERS)}"
)
...
print(
    f"{'Class':<20} {'Layer':<8} {'Baseline COMPLY':<18} {'Flipped':<10} {'Coherent'}"
)
print("-" * 70)
...
for cls_name in JB_CLASSES:
    for layer in TEST_LAYERS:
        ...
        print(
            f"  {cls_name:<18} L{layer:<6} {comply}/{len(cls_layer):<16} {flipped}/{comply if comply else len(cls_layer):<8} {coherent}"
        )
...
for layer in TEST_LAYERS:
    all_layer = [
        r
        for r in intervention_results
        if r.get("layer") == layer and "intervened_cls" in r
    ]
    ...
    print(
        f"  L{layer:>2}: {comply} comply, {flipped} flipped ({pct:.0f}%), {coherent} coherent"
    )
