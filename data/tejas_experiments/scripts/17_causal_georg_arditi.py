...
def is_coherent(resp):
    ...
    words = resp.split()
    if len(words) > 10 and len(set(words)) / len(words) < 0.2: return False
...
if arditi_path.exists():
    ...
    with open(arditi_path) as f:
        arditi = json.load(f)
    arditi_int = [r for r in arditi["intervention_results"] if r.get("layer") == BEST_LAYER and "intervened_cls" in r]
