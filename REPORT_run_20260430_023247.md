...
9. **Jailbreaks edit the residual stream along *r̂* — confirmed both geometrically and causally** (added 2026-05-04). **Geometrically**, at pos=−2 (the model token where the refusal/comply decision lives), the empirical "JB direction" *r_jb* = mean(*h_bare*) − mean(*h_jb_class*) has cosine similarity **+0.72 to +0.94** with *r̂* across all five classes (n=50 each); magnitudes range 0.40 to 1.11 ‖*r̂*‖, with cognitive_reframe at exactly **1.04 ‖*r̂*‖**. **Causally**, an intervention that adds the universal mean *r_jb_universal* (cos = +0.925, ‖·‖ = 0.65·‖*r̂*‖) to JB-comply prompts at L15 every forward pass flips **47/89 = 52.8 %** [Wilson 95 % CI 42.5, 62.8] back to refuse — substantially higher than the strongest sparse-feature ablation (31.5 % [22.8, 41.7]; non-overlapping CIs) despite the empirical *r_jb* being a *dose-reduced* (0.65×) approximation of *r̂*. Stage 06's full-dose Arditi r̂ reaches 100 % at 1.0·‖*r̂*‖; the per-class flip rate is inversely related to per-class r_jb magnitude (universal vector under-doses cognitive_reframe at 27.3 % flip, over-doses fiction at 73.7 %), pointing to per-class *r_jb_class* interventions as the natural rigorous follow-up. **Headline takeaway: bottleneck is basis, not dimensionality.** A 1-D vector aligned with *r̂* — whether the canonical Arditi extraction or an empirically-derived JB displacement — beats thousands of transcoder features along the same axis. See § 5.5 (geometry) and § 5.6 (causal).
...
Construct the per-class *jailbreak direction* in residual space:

> *r_jb_cls* = mean(*h_bare_p*) − mean(*h_jb_cls_p*) at pos=−2

This is the displacement vector that the JB applies on average to the residual at the decision token. Cosine similarity with *r̂*:

| Class | cos(*r̂*, *r_jb*) | ‖*r_jb*‖ / ‖*r̂*‖ | cos(*r̂*, *r_jb_sem*) | ‖*r_jb_sem*‖ / ‖*r̂*‖ |
|---|---:|---:|---:|---:|
| fiction | **+0.72** | 0.49 | +0.36 | 0.33 |
| roleplay | **+0.89** | 0.65 | −0.14 | 0.17 |
| analytical | **+0.94** | 0.78 | **+0.83** | 0.32 |
| completion | **+0.80** | 0.40 | −0.71 | 0.23 |
| cognitive_reframe | **+0.94** | 1.11 | **+0.92** | 0.47 |

Where *r_jb_sem* = mean(*h_ctrl_cls*) − mean(*h_jb_cls*) is the *prefix-controlled* (semantic-only) JB direction.

**Two findings:**

1. **The full JB direction (bare → jb) is highly aligned with *r̂* at pos=−2 across all five classes** (cos = +0.72 to +0.94). This is the direct geometric answer to "do jailbreaks edit *r̂*?" — yes; the residual displacement they induce is approximately parallel to *r̂*, not just projecting onto it. Analytical and cognitive_reframe are nearly co-linear with *r̂* (cos = 0.94).

2. **The semantic-only JB direction (ctrl → jb) is strongly aligned with *r̂* for analytical and cognitive_reframe** (cos = +0.83 and +0.92 respectively) but **anti-aligned or weakly aligned for roleplay, fiction, completion**. This refines § 5.3's "most of the JB effect is prefix-induced" finding: when there *is* a semantic JB effect (analytical, cognitive_reframe), it lives on the same axis as the prefix effect — both push along *r̂*. For the other three classes the semantic component is weaker and points elsewhere.
...
Per-position breakdown for cognitive_reframe (the strongest class at pos=−2; sign convention: cos(*r̂*, *r_jb*) where *r_jb* = bare − jb, and Δ jb − bare in |*r̂*| units shows where jb sits relative to bare on the r̂ axis):

| Position | cos(*r̂*, *r_jb*) | Δ jb − bare in \|*r̂*\| units | Direction of JB shift |
|---|---:|---:|---|
| −5 (template) | **−0.84** | **+0.94** | jb > bare on r̂ axis (counter to the pos=−2 effect) |
| −3 (template) | **−0.69** | +0.29 | jb > bare (smaller) |
| **−2 (model token)** | **+0.94** | **−1.04** | jb < bare (the refusal-suppression effect) |
| avg of [−5, −3, −2] | −0.27 | −0.06 | not meaningful (cancellation across positions) |

The sign of the JB-induced *r̂*-projection shift **flips between template positions and the decision position**. At template positions inside the JB prefix, the residual is *more* pro-refusal-aligned than bare's template positions; at the model token, it has *swung past* bare to land on the anti-refusal side. This is consistent with a "JB sets up pro-refusal features early and over-corrects them later" picture of the mechanism, and is one motivation for the edge-ablation experiment Georg suggested.

For the simple paper claim that JBs edit *r̂*, **the relevant measurement is pos=−2 only** — the position where the refuse/comply decision is geometrically encoded. Cross-position averaging or measurements at template positions obscure the alignment because of the sign flip above.

#### 5.5.4 Closing the loop with Stage 06

The three measurements together form a tight causal chain:

| Measurement | Value | What it shows |
|---|---:|---|
| JB displacement magnitude on *r̂* (cognitive_reframe) | **−1.04 ‖*r̂*‖** | JBs edit by ~1 ‖*r̂*‖ |
| cos(*r̂*, empirical JB direction) | **+0.94** | The edit is parallel to *r̂* |
| Stage 06 +1.00 ‖*r̂*‖ pro-refusal-add flip rate | **89/89 = 100 %** | Adding back +1 *r̂* fully reverses the edit |
| Stage 06 −1.00 ‖*r̂*‖ anti-refusal-sub flip rate | **49/50 = 98 %** | Subtracting 1 *r̂* on bare reproduces the JB edit |

This is the strongest formulation we have of "jailbreaks edit the refusal direction": the empirical edit magnitude, the empirical edit direction, and the synthetic intervention that reverses it all agree at the ~1 ‖*r̂*‖ scale and on the *r̂* axis at pos=−2.
...
The cosine alignment in § 5.5.2 (cos = +0.72 to +0.94 across classes at pos=−2) is geometric / correlational. It does not by itself distinguish

- **(A)** JBs shift the residual exactly along *r̂* (cos → 1; the orthogonal component is zero), from
...
We use a single **universal jailbreak vector**

> *r_jb_universal* = mean over the 5 JB classes of *r_jb_class* = mean over classes of [mean(*h_bare*) − mean(*h_jb_class*)] at L15 pos=−2
...
| **Experiment A: +*r_jb_universal*** (empirical, 0.65·‖*r̂*‖) | **52.8 %** | 89 | **[42.5, 62.8]** | 100.0 % |
| **Experiment B: −*r_jb_universal*** (empirical, 0.65·‖*r̂*‖) | **16.0 %** | 50 | **[8.3, 28.5]** | 98.0 % |

Coherence: 89/89 (Exp A) and 50/50 (Exp B) — neither intervention broke generation.

**Per-class breakdown of Experiment A** (the more interpretable one):

| JB class | r_jb_class magnitude | cos(*r̂*, *r_jb_class*) | Exp A flip rate |
|---|---:|---:|---:|
...
**Experiment B (16 % flip rate on bare-refuse) is starkly asymmetric to Experiment A.** Subtracting the universal vector at 0.65·‖*r̂*‖ does not flip bare-refuse to comply nearly as effectively as adding it does the inverse on JB-comply. Two contributing explanations, both consistent with the data:
...
For the workshop-paper claim "JBs edit *r̂*", Experiment A is the load-bearing measurement; Experiment B's lower flip rate is consistent with the dose-asymmetry above and does *not* undermine the directional-alignment claim.
...
|---|---:|---|
| cos(*r̂*, *r_jb_universal*) at pos=−2 | **+0.925** | r_jb is essentially parallel to r̂ (only ~6° off-axis) |
...
| **+1.0·*r_jb_universal* flip rate** | **52.8 %** | Empirical *r_jb* at empirical dose recovers majority |
...
> **Across every method we tested, the bottleneck on jailbreak recovery is *basis*, not *dimensionality*.** A 1-D vector aligned with *r̂* — whether the canonical Arditi extraction (100 % flip at 1·‖*r̂*‖) or an empirically-derived JB-displacement vector (52.8 % flip at 0.65·‖*r̂*‖) — outperforms thousands of transcoder features along the same axis (≤35 % at best per-prompt subcircuit). The directional component is causally sufficient; sparse feature reconstruction is causally insufficient.
