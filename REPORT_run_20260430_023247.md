...
### 12.7 Per-prompt vs corpus-union construction needs replication

The 9.9 % → 23.7 % recovery jump from changing only the subcircuit construction rule (with feature count *decreasing* from 88 to 6) is large. We have one such measurement on one dataset on one model. The paper-grade story requires (i) running this comparison on the prior run `run_20260422_015552` for replication, (ii) showing the same effect under `--positions anchors`, and (iii) verifying `L13:F427` is the same numerically after baseline normalization to the Stage 06 reference (§ 9.7.5).
...
1. ~~Ablate `canonical_pro_refusal`~~ — **done** (§ 9.7). The sweep added the methodological per-prompt-vs-corpus-union finding to the headline list.
2. **Replicate the per-prompt-vs-corpus-union sweep on `run_20260422_015552`.** Same 3 Stage 07 configs already exist on the prior run; cloning the sister-dir trick yields a within-paper replication that strengthens the methodology claim. ~6–7 h on the local 4090, no RunPod required.
3. **Normalize cross-run baselines** (§ 9.7.5). Either re-run the Stage 08 canonical sweep with `--max-new-tokens 200` (matches Stage 06) or post-hoc reclassify shorter generations with the bulletproof Phase-0 baselines as ground truth. The latter is cheaper.
4. **Stand up the live ablation server + frontend cart** for ad-hoc per-feature/supernode interactive ablation. The infrastructure is ~70 % built (`scripts/pipeline/ablation_server.py` + `05_frontend_patches/feature-cart.js`); needs end-to-end testing + extension to compare.html + supernode-grouping UI. See companion plan `FRONTEND_ABLATION_PLAN.md`.
5. **Sweep Stage 08 across (K, F) configurations on the JB-class-specific subcircuits** to resolve the negative-dissociation finding (§ 9.3). The canonical sweep already proved the (K, F) sweep is informative; extending it to the per-class subcircuits is mechanical.
6. **Run `--positions anchors`** on the existing run dir for the position-mode comparison.
7. **Replace `jb_completion`** in the dataset with a bypass-style JB before any camera-ready.
8. **Wire attention-head attribution** as a Stage 09 — circuit-tracer supports it, the wiring is the only barrier.
