...
9. **Jailbreaks edit the residual stream along *r̂*** (added 2026-05-04, direction alignment analysis). At pos=−2 (the model token, where the refusal/comply decision is geometrically encoded), the empirical "JB direction" *r_jb* = mean(*h_bare*) − mean(*h_jb_class*) has cosine similarity **+0.72 to +0.94** with *r̂* across all five classes (n=50 each). Magnitudes range from **0.40 ‖*r̂*‖ to 1.11 ‖*r̂*‖**; cognitive_reframe sits at exactly **−1.04 ‖*r̂*‖** of displacement along *r̂*. Stage 06's synthetic +1.00 ‖*r̂*‖ intervention reverses the displacement at 89/89 = 100 %; subtracting 1.00 ‖*r̂*‖ on bare reproduces it at 49/50 = 98 %. **The empirical JB direction, the synthetic intervention magnitude, and the synthetic intervention axis all agree on the same scalar (~1 ‖*r̂*‖) and the same axis (*r̂* at pos=−2).** See § 5.5.
...
#### 9.9.4 Per-class JB recovery on the Pareto curve (k100_f20 vs Tier 1)

| N | fiction | roleplay | analytical | cognitive_reframe |
|---:|---:|---:|---:|---:|
| top_1   |  15.8 % | 11.1 %   |  0.0 %     |  3.0 %            |
| top_5   |  26.3 % | 33.3 %   | 10.7 %     |  0.0 %            |
| top_10  |  31.6 % | **77.8 %** |  7.1 %   |  3.0 %            |
| top_20  |  42.1 % | **66.7 %** | 21.4 %   |  9.1 %            |
| **top_50** | **36.8 %** | **88.9 %** | **25.0 %** | (incomplete in extracted log)  |

Roleplay saturates first (88.9 % at top_50), consistent with its small denominator (n_jb_comply = 9; all of roleplay collapses with the right 50 features). Fiction caps near 42 % — even the per-prompt top-N method cannot reach the 100 % direction-recovery on fiction, again pointing to fiction's effect living in something MLP-feature ablation cannot fully reach (residual-stream attention, embedding-level differences, or attribution-graph nodes outside the transcoder set).
