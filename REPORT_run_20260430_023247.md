...
9. **Jailbreaks edit the residual stream toward the harmless direction — confirmed both geometrically and causally** (added 2026-05-04, sign convention follows Ball 2024 / Wang 2025: *r_jb* points TOWARD jailbreak). **Geometrically**, at pos=−2 (the model token where the refusal/comply decision lives), the empirical jailbreak direction *r_jb_class* = mean(*h_jb_class*) − mean(*h_bare*) has cosine similarity **+0.72 to +0.94 with the harmless direction (−*r̂*)** across all five classes (n=50 each) — equivalently, **−0.72 to −0.94 with *r̂*** (anti-parallel to refusal). Magnitudes range 0.40 to 1.11 ‖*r̂*‖, with cognitive_reframe at exactly **1.04 ‖*r̂*‖** of harmful→harmless displacement. This is the geometric form of "JBs make the model perceive the prompt as harmless." **Causally**, an intervention that **subtracts** the universal mean *r_jb_universal* (cos with −*r̂* = +0.925, ‖·‖ = 0.65·‖*r̂*‖) from JB-comply prompts at L15 every forward pass — i.e. removes the empirical jailbreak displacement — flips **47/89 = 52.8 %** [Wilson 95 % CI 42.5, 62.8] back to refuse. Substantially higher than the strongest sparse-feature ablation (31.5 % [22.8, 41.7]; non-overlapping CIs) despite the empirical *r_jb* being a *dose-reduced* (0.65×) approximation of ‖*r̂*‖. Stage 06's full-dose Arditi *r̂* intervention reaches 100 % at 1.0·‖*r̂*‖. The per-class flip rate is inversely related to per-class ‖*r_jb_class*‖ (universal vector under-doses cognitive_reframe at 27.3 % flip, over-doses fiction at 73.7 %), pointing to per-class *r_jb_class* interventions as the natural rigorous follow-up. **Headline takeaway: bottleneck is basis, not dimensionality.** A 1-D vector aligned with the harmless axis — whether the canonical Arditi extraction or an empirically-derived JB displacement — beats thousands of transcoder features along the same axis. See § 5.5 (geometry) and § 5.6 (causal).
